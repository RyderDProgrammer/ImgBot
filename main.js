...
client.on("message", (msg) => {
  ...
  if (
    urlLink[0].length > 1 &&
    vidInArray(urlLink[0]) &&
    !urlLink[0].includes(spoiler)
  ) {
    fs.appendFile(
      "./UsersVideos/" + msg.author.id + "vid.txt",
      "\n" + urlLink,
      (err) => {
        if (err) throw err;
      }
    ...
    vidLink.includes("https://twitter") ||
    vidLink.includes("https://www.youtube") ||
    vidLink.includes("https://youtu.be") ||
    vidLink.includes("https://cdn") ||
    (vidLink.includes("gif") && !vidLink.includes(spoiler))
  ) {
    fs.appendFile(
      "./UsersVideos/" + msg.author.id + "vid.txt",
      "\n" + msg.content,
  ...
  if (msg.content.startsWith(prefix)) {
    //Bot cannot enter any channels that are marked as NSFW
    if (msg.channel.nsfw) return;
    if (msg.content.startsWith(prefix + "commands")) {
      msg.channel.send(
        "Use !username or !usernick and i-image or v-video to grab a random video or image from that user."
      );
      msg.channel.send(
        "Like this !X i or !X img will both pull an image from X's file."
      );
      return;
    } else if (
    /*
      ...
      userIdArray.includes(usernameInIDArray(msgUserName)) &&
      validImgQuery.includes(imgOrVid)
    ) {
      if (validImgQuery.indexOf(imgOrVid) < 5) {
        let grabImg =
          "./UsersImages/" + usernameInIDArray(msgUserName) + "img.txt";
        let sendImg = randomLineInFile(grabImg);
        if (sendImg != "") msg.channel.send(sendImg);
      } else {
        let grabVid =
          "./UsersVideos/" + usernameInIDArray(msgUserName) + "vid.txt";
    ...
    //so that way I know which xInIDArray to query.
      ...
      userIdArray.includes(nicknameInIDArray(msgUserName)) &&
      validImgQuery.includes(imgOrVid)
    ) {
      if (validImgQuery.indexOf(imgOrVid) < 5) {
        let grabImg =
          "./UsersImages/" + nicknameInIDArray(msgUserName) + "img.txt";
        let sendImg = randomLineInFile(grabImg);
        if (sendImg != "") msg.channel.send(sendImg);
      } else {
        let grabVid =
          "./UsersVideos/" + nicknameInIDArray(msgUserName) + "vid.txt";
        let sendVid = randomLineInFile(grabVid);
        if (sendVid != "") msg.channel.send(sendVid);
      }
    } else {
  ...
  for (let i = 0; i < userIdArray.length; i++) {
    ...
    fs.stat(fileExist, function (stat) {
      if (stat) {
        ...
        fs.writeFile(
          path.join(__dirname, "/UsersImages", currUser),
          "",
          (err) => {
            if (err) throw err;
          }
  ...
  for (let i = 0; i < userIdArray.length; i++) {
    ...
    fs.stat(fileExist, function (stat) {
      if (stat) {
        ...
        fs.writeFile(
          path.join(__dirname, "/UsersVideos", currUser),
          "",
          (err) => {
            if (err) throw err;
          }
